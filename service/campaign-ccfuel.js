...
log4js.configure({
  appenders: { VMDropRequest: { type: "file", filename: "public/debug.log" } },
  categories: { default: { appenders: ["VMDropRequest"], level: "debug" } }
});

const logger = log4js.getLogger("VMDropRequest");

const { Campaign } = require('../models/campaign');
const { CallHistory } = require('../models/call-history');
const { PUBLIC_FOLDER_NAME, ASSET_FOLDER_PATH, VMDROP_URL, VARHTML, CCFUEL_API_URL, API_KEY, CCFUEL_API_KEY, LOCAL_URL, PROD_URL, CALLBACK_PATH } = require('../global/constants');
const { generateRandomId, isEmpty } = require('../utils');

const makeCampaignCallParams = (campaign, number) => {
  console.log("CAMPAIGN AUDIO FILE NAME ===>" + PROD_URL + '/' + PUBLIC_FOLDER_NAME + ASSET_FOLDER_PATH + campaign.audioFileName);
  return {
    // audio_url: LOCAL_URL + ASSET_FOLDER_PATH + campaign.audioFileName, // TODO,hard coded
    audio_url: PROD_URL + '/' + PUBLIC_FOLDER_NAME + ASSET_FOLDER_PATH + campaign.audioFileName,
    lead_phone: number,
    missed_call: true, //TODO  campaign.missedCalls,
    ...
    callback_url: PROD_URL + ':3012' + CALLBACK_PATH,
...
exports.runCampagins = async timer => {
  console.log('Gobil==> cron job CCFUEL VRM start count ', timer)
  ...
  campaigns.forEach(async campaign => {
    ...
    if (campaign.campaignRepeatCount <= 1) {
      ...
      fs.createReadStream(VARHTML + PUBLIC_FOLDER_NAME + ASSET_FOLDER_PATH + campaign.csvFileName)
        .pipe(csv())
        .on('data', data => {
        ...
        .on('end', () => {
          ...
          numbers.forEach((number, index) => {
            console.log('GOBIL VM DROP', index);
...
const makeVMDropRequest = async (params, campaignId, number, ccfuelAPI) => {
  ...
  console.log(CCFUEL_API_URL, params);
  logger.debug(CCFUEL_API_URL + '?apikey=' + CCFUEL_API_KEY, JSON.stringify(params))
  axios.post((ccfuelAPI ? CCFUEL_API_URL : VMDROP_URL) + "?apikey=" + (ccfuelAPI ? CCFUEL_API_KEY : API_KEY), params)
    .then(async response => {
      ...
      const history = {
        ...
        status: response.data.status ? response.data.status === 'Failed' ? 'Failed' : 'Pending' : 'Pending',
        carrier: response.data.carrier
      };
      try {
        logger.debug('VMDROPREQUEST RESPONSE', response.data ? JSON.stringify(response.data) : response.data);
      } catch (er) {

      }

      CallHistory.addCallHistory(history);
      // const _campaign = await Campaign.findOne({ _id: campaignId });

      // Campaign.editCampaign({
      //   _id: campaignId,
      //   responseIndex: (_campaign.responseIndex || 0) + 1
      // });
    })
    .catch(async error => {
      ...
      CallHistory.addCallHistory(history);
      try {
        logger.debug('VMDROPREQUEST ERROR', error.data ? JSON.stringify(error.data) : error.data);
      } catch (er) {
